...
of the `App` class has a reference to the proxy. This means that method calls on that
...
We use the following command to run the `App` class shown earlier:
...
The output from the execution of the `App` program looks something like the next example.
...
Spring beans. The following slight variation on the `App` program yields the same
result:
...
`annotation` namespace. The following example shows how to use `<context:load-time-weaver/>`:
