...
of XML namespaces provided by Spring -- for example, the `annotation` and `util` namespaces.
...
With the `annotation` namespace introduced in Spring 2.5, you can configure property placeholders
...
With the `annotation` namespace introduced in Spring 2.5, it is possible to configure
...
configuration (notice the inclusion of the `annotation` namespace):
...
----
<1> The `annotation` field is injected based on the known resolvable dependency type:
`ApplicationContext`.
